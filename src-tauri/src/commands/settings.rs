...
pub struct Settings {
    ...
    /// Retained for settings-file compatibility; Saleem's AI Factory should not
    ...
    /// Retained for settings-file compatibility; Saleem's AI Factory does not shell
    ...
    /// for settings-file compatibility; Saleem's AI Factory currently reads metadata
...
pub(crate) async fn persist(app_data_dir: &Path, mut settings: Settings) -> Result<Settings, AppError> {
    ...
    // that's never run Saleem's AI Factory could plausibly hit this otherwise.
