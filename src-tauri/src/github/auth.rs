...
pub const KEYCHAIN_SERVICE: &str = "com.saleem.saleems-ai-factory-app";
...
/// Saleem's AI Factory GitHub OAuth App under `saleemmbayed-sketch`'s account.
...
fn build_oauth_client() -> Result<reqwest::Client, AppError> {
    reqwest::Client::builder()
        ...
        .user_agent(concat!(
            ...
            " (+https://github.com/saleemmbayed-sketch/saleems-ai-factory-app)"
        ))
        .build()
        .map_err(|e| AppError::Network {
            url: DEVICE_CODE_URL.into(),
...
mod tests {
    ...
    fn keychain_constants_are_stable() {
        ...
        // Renamed source-app service ID → com.saleem.saleems-ai-factory-app
        // on the 2026-06-05 rebrand fork; matches tauri.conf.json identifier.
        assert_eq!(KEYCHAIN_SERVICE, "com.saleem.saleems-ai-factory-app");
