...
//!
//! Ports saleems-ai-factory `scripts/convert.sh`. Every renderer is a PURE function
...
/// Map a saleems-ai-factory `color` (named or hex) to an OpenCode-safe `#RRGGBB`
...
mod tests {
    ...
    #[test]
    #[ignore = "requires SALEEMS_AI_FACTORY_PARITY_ROOT and executes upstream convert.sh"]
    fn upstream_convert_sh_is_byte_identical_for_transform_tools() {
        let root = std::env::var("SALEEMS_AI_FACTORY_PARITY_ROOT")
            .expect("set SALEEMS_AI_FACTORY_PARITY_ROOT to a saleems-ai-factory clone");
