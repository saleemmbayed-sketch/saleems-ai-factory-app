...
//! ```text
//! <app_data_dir>/
//! ├── corpus/                 our maintained copy of the Saleem's AI Factory repo
//! │   └── <category>/<slug>.md
//! └── state/
...
/// (the canonical division truth the Saleem's AI Factory repo declares, shared with
...
/// the bundled floor (`catalog-categories.json`, itself a mirror of the catalog's
/// `divisions.json`).
///
...
const CATALOG_GIT_URL: &str = "https://github.com/saleemmbayed-sketch/saleems-ai-factory.git";

/// Dev-root directory names scanned (under `$HOME`) by the "Find Saleem's AI Factory"
...
const USER_AGENT: &str = "saleems-ai-factory/0.1 (+https://github.com/saleemmbayed-sketch/saleems-ai-factory)";
...
pub struct Corpus {
    ...
    /// the bundled `catalog-categories.json` floor (see [`load_division_meta`]).
...
const CATEGORIES_JSON: &str = include_str!("../../data/catalog-categories.json");
const DIVISIONS_FILENAME: &str = "divisions.json";

/// The bundled `catalog-categories.json` parsed into a slug → row map. This is
...
async fn refresh(app_data_dir: &Path) -> Result<CorpusMeta, AppError> {
    ...
    // The tarball has a single top-level `saleems-ai-factory-main/` prefix we strip.
    let dir = catalog_root(app_data_dir, &source);
...
/// everything under a single `saleems-ai-factory-main/` top-level dir, which we
...
fn extract_categories(tar_gz: &[u8], dest: &Path, categories: &[String]) -> Result<u32, AppError> {
    ...
    for entry in entries {
        ...
        // Strip the single top-level `saleems-ai-factory-main/` component.
        let comps: Vec<String> = path
            .components()
            .filter_map(|c| match c {
                std::path::Component::Normal(s) => s.to_str().map(|s| s.to_string()),
...
/// `~/.saleems-ai-factory` — the default managed-catalog location (shared with the
/// `saleems-ai-factory` CLI).
fn home_agency_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".saleems-ai-factory"))
}
...
/// Detect candidate catalogs. Always checks `~/.saleems-ai-factory`; when `scan` is
/// true also walks common dev roots for a `saleems-ai-factory` checkout (the "Find
/// Saleem's AI Factory" button). Pure of app state — safe to call anytime.
    ...
    if scan {
        if let Some(home) = dirs::home_dir() {
            for root in SCAN_ROOTS {
                // Look for `<home>/<root>/saleems-ai-factory` and a direct
                // `<home>/<root>` that is itself a catalog.
                let base = home.join(root);
                push(candidate_for(&base.join("saleems-ai-factory"), "userClone"), &mut candidates, &mut seen);
                // One level of children named with "saleems" (cheap heuristic).
                if let Ok(rd) = std::fs::read_dir(&base) {
                    for ent in rd.filter_map(|e| e.ok()) {
                        let p = ent.path();
                        if p.is_dir() && p.file_name().map(|n| n.to_string_lossy().contains("saleems")).unwrap_or(false) {
...
/// Ensure `~/.saleems-ai-factory` holds a catalog, cloning (git) or unpacking the
...
pub async fn catalog_source_set(
    ...
    if let CatalogSource::Managed { path } | CatalogSource::UserClone { path, .. } = &source {
        ...
        if !looks_like_catalog(&root) {
            return Err(AppError::InvalidArgument {
                message: format!(
                    "{path} doesn't look like a Saleem's AI Factory catalog (no scripts/convert.sh or category dirs)"
                ),
            });
        }
    }
...
/// `~/.saleems-ai-factory`; `scan=true` also walks common dev roots).
#[tauri::command]
pub async fn catalog_detect(scan: bool) -> Result<CatalogDetection, AppError> {
    Ok(detect_catalogs(scan).await)
}

/// `catalog_provision_managed()` — clone/snapshot into `~/.saleems-ai-factory`, set
...
/// Heuristic: does `root` hold a Saleem's AI Factory catalog? True if it has the
...
mod tests {
    ...
    /// (`catalog-categories.json`, a mirror of the catalog's `divisions.json`), so
    /// `strategy/` (playbooks/runbooks) is NOT a division and `integrations/`
    /// (convert.sh output) is NOT either. Counts are pinned to the Saleem's AI Factory
...
"#;
    ...
    async fn catalog_source_persists_and_defaults_bundled() {
        ...
        let src = CatalogSource::Managed { path: "/Users/x/.saleems-ai-factory".into() };
    ...
    fn catalog_root_resolves_per_source() {
        ...
        assert_eq!(
            catalog_root(app_data, &CatalogSource::Managed { path: "/home/x/.saleems-ai-factory".into() }),
            PathBuf::from("/home/x/.saleems-ai-factory")
        );
        assert_eq!(
    ...
    fn discover_categories_falls_back_to_bundled_floor_without_divisions_json() {
        ...
        // No divisions.json → the bundled floor (catalog-categories.json) keys.
